...
import { providers, mistralModels, listModels, loadModel, getApiKeyName } from './index';
...
describe('AI SDK Model Picker', () => {
  ...
  describe('listModels function', () => {
    ...
    test('should handle provider synonyms', () => {
      const result = listModels({ provider: 'mistral' });
      
      assert.strictEqual(result.length, 1);
      assert.strictEqual(result[0].provider, 'mistralai');
    });
